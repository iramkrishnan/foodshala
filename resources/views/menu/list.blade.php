...
@section('content')
    ...
    <script type="text/javascript">
        $('#search').on('keyup', function () {
            ...
            // add hide in food body if it doesn't already exist
            if (!document.getElementById('food-body').classList.contains('hide')) {
                document.getElementById("food-body").classList.toggle("hide");
            }
            ...
            if (document.getElementById("search").value === '') {
                ...
                document.getElementById("food-body").classList.toggle("hide"); // remove hide
            }
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        row += '                <div class="lg:w-1/4 md:w-1/2 p-4 w-full">\n' +
                            '                    <a class="block relative h-48 rounded overflow-hidden" href="/menu/' + data[i]['slug'] + ' ">\n' +
                            '                        <img alt="" class="object-cover object-center w-full h-full block"\n' +
                            '                             src="https://source.unsplash.com/421x261/?food,' + data[i]['menu_item'] + ' ">\n' +
                            '                    </a>\n' +
                            '                    <div class="mt-4">\n' +
        ...
        function myFunction() {
            ...
            if (document.getElementById("food-body").classList.contains("hide")) {
                document.getElementById("food-body").classList.toggle("hide"); // remove hide in food body

            }
            if (!document.getElementById("food-body").classList.contains('show')) {
                document.getElementById("food-body").classList.toggle("show"); // add show in food body
            }
                        ...
                        <div class="lg:w-1/4 md:w-1/2 p-4 w-full">
                            ...
                            <div class="mt-4">
                                <h3 class="text-gray-500 text-xs tracking-widest title-font mb-1"><a
                                        href="/menu/{{$menuItem->slug}}">Food</a></h3>
                                <h1 class="text-gray-900 title-font text-lg font-medium"><a
                                        href="/menu/{{$menuItem->slug}}">{{$menuItem->menu_item}}</a></h1>
                            </div>
                        </div>
